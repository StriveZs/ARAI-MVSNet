...
class MVSTrainSet(Dataset):
    def __init__(self, root_dir, data_list, lightings=range(7), num_views=4, depth_num=64):
    ...
    def load_depths(self, path):
        depth_s1 = np.array(read_pfm(path)[0], np.float32)
        h, w = depth_s1.shape
        depth_s2 = cv2.resize(depth_s1, (w * 2, h * 2), interpolation=cv2.INTER_CUBIC)
        depth_s3 = cv2.resize(depth_s1, (w * 4, h * 4), interpolation=cv2.INTER_CUBIC)
        depth_s0 = cv2.resize(depth_s1, (w // 2, h // 2), interpolation=cv2.INTER_CUBIC)
        return {'stage1': depth_s0, 'stage2': depth_s1, 'stage3': depth_s2, 'stage4': depth_s3}
    ...
    def __getitem__(self, idx):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            if i == 0:
                dep_path = '{}/Depths/{}_train/depth_map_{:04d}.pfm'.format(self.root_dir, scene_name, idx)
        ...
        proj_mats_s0 = proj_mats_s3.copy()
        proj_mats_s0[:, 1, :2, :3] = proj_mats_s3[:, 1, :2, :3] / 8.
        proj_mats = {'stage1': proj_mats_s0, 'stage2': proj_mats_s1, 'stage3': proj_mats_s2, 'stage4': proj_mats_s3}
...
class MVSTestSet(Dataset):
    def __init__(self, root_dir, data_list, max_h, max_w, num_views=7, depth_num=64):
